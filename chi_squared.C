...
for (double scale = 0.950; scale <= 1.050; scale += 0.0005) {
    // Format the histogram name with 4 decimal places
    std::ostringstream oss;
    oss << std::fixed << std::setprecision(4) << scale;
    std::string histName = "h3MassFlavorPairs_DATAMC_scale_" + oss.str();
...
<< ", x_values_max " << x_values_max << endl << flush;
    ...
    gPad->Update();
    gPad->RedrawAxis();
    c1->SaveAs("pdf/chi_squared_vs_scale_factor.pdf");
}