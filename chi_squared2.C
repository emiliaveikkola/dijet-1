...
void chi_squared2() {
    ...
    for (size_t i = 0; i < scaleFactors.size(); ++i) {
        for (size_t j = 0; j < scaleFactors.size(); ++j) {
            ...
            std::string histName = "h3MassFlavorPairs_DATAMC_" + oss.str();
        ...
        h3all_scaled->Scale(scale_factor2);
            ...
            chi2_hist->Fill(scale1, scale2, chi27m);
...
c2->SetLogz();
chi2_hist->Draw("COLZ");
chi2_hist->GetZaxis()->SetMoreLogLabels();
chi2_hist->GetZaxis()->SetNoExponent();

TLine *l = new TLine();
l->DrawLine(0.95,0.95,1.05,1.05);
...
for (int i = 1; i != chi2_hist->GetNbinsX()+1; ++i){
    double chi2 = chi2_hist->GetBinContent(i,i);
    double x = chi2_hist->GetXaxis()->GetBinCenter(i);
    g2->SetPoint(i-1,x,chi2);
    if (chi2 < chi2_min){
        chi2_min = chi2;
        x_min = chi2_hist->GetXaxis()->GetBinCenter(i);
        y_min = chi2_hist->GetYaxis()->GetBinCenter(i);
    }
}
...
// Save the canvas
c2->SaveAs("pdf/chi_squared_2D_correct.pdf");


// Save the chi2_hist to a ROOT file
TFile* outputFile = new TFile("chi2_hist.root", "RECREATE");
chi2_hist->Write();
g2->Write();
outputFile->Close();
