...
float ctag1, ctag2;
    ...
    TFile* output_file_ud = new TFile("output_stag_udsample.root", "RECREATE");
    TTree* ud_tree = tree->CloneTree(0); // Clone tree structure without copying events
   ...
   // Write the output tree and close files
    output_file_ud->cd(); // Ensure you are in the correct directory
    ud_tree->Write();
    output_file_ud->Close();
