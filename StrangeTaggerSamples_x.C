...
float ctag1, ctag2;
    ...
    TFile* output_file_x = new TFile("output_stag_xsample.root", "RECREATE");
    TTree* x_tree = tree->CloneTree(0); // Clone tree structure without copying events
   ...
   const int nlap2 = 80000;
    ...
    for (Long64_t jentry = 0; jentry < nentries; jentry++) {
      ...
      if (time>0) cout << Form("\n\%1.0f ev/s\n",nlap2/time) << flush;
        ...
        if ((ctag1 >= 0.06 || ctag2 >= 0.06) && (ctag1 <= 0.43 && ctag2 <= 0.43)) {
            x_tree->Fill();
        }
    }

   // Write the output tree and close files
    output_file_x->cd(); // Ensure you are in the correct directory
    x_tree->Write();
    output_file_x->Close();
